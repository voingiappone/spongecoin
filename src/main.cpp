...
uint256 hashGenesisBlock("0xa01b11556feff0d245b318edf9c0d8824fabceade67ea6c4891ea3f38146e72e"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SpongeCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SpongeCoin Signed Message:\n";
...
{
    // SpongeCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // SpongeCoin: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // SpongeCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SpongeCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, spongecoin - DarkGravity v2, written by Evan Duffield - evan@spongecoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, spongecoin - DarkGravity v3, written by Evan Duffield - evan@spongecoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // SpongeCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // SpongeCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "verack")
    {
        ...
        if((fTestNet && addr.GetPort() != 18009) || (!fTestNet && addr.GetPort() != 8008)) return true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SpongeCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SpongeCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SpongeCoinMiner(CWallet *pwallet)
{
    printf("SpongeCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("spongecoin-miner");
    ...
    try { loop {
        ...
        printf("Running SpongeCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SpongeCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SpongeCoinMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - DARKCOIN **********
    Copyright 2014, Spongecoin Developers 
        eduffield - evan@spongecoin.io
        InternetApe - kyle@spongecoin.io
*/
